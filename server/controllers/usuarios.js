...
module.exports = {
	register(req, res) {
		return Usuario.findOne({
			...
			if (usuario) {
				...
				if (req.body.senha == '' || req.body.senha == null || (req.body.nome == '' || req.body.nome == null))
	...
	list(req, res) {
		return Usuario.findAll({ attributes: [ 'id', 'nome', 'email', 'createdAt', 'updatedAt' ] })
	...
	update(req, res) {
		return Usuario.find({ where: { id: req.params.id } }).then((usuario, err) => {
			if (err) res.status(400).send({ message: err + '' });
			if (usuario) {
	...
	remove(req, res) {
		return Usuario.find({ where: { id: req.params.id } })
			.then((usuario) => {
				if (usuario) {
